...
public class KeyboxImitationHooks {
    ...
    private static final boolean DEBUG = SystemProperties.getBoolean("persist.sys.pihooks.gms_key_attestation_DEBUG", true);
    ...
    private static boolean isSpoofingGMS() {
         if (SystemProperties.getBoolean("persist.sys.pixelprops.gms", true) && isCallerSafetyNet()) {
            return true;
         } else {
            return false;
         }
    ...
    public static KeyEntryResponse onGetKeyEntry(KeyEntryResponse response) {
        ...
        if (!sEnableKeyAttestationSpoof) {
            Log.d(TAG, "Key attestation spoofing is disabled by user");
            blockKeyAttestationIfSpoofingGMS(1);
            return response;
        }
        ...
        if (!KeyProviderManager.isKeyboxAvailable()) {
            ...
            blockKeyAttestationIfSpoofingGMS(2);
            return response;
        }
        ...
        try {
            if (response.metadata.certificate == null) {
                Log.w(TAG, "Certificate is null, skipping modification");
                return response;
            }
            ...
            if (certificate.getExtensionValue(KEY_ATTESTATION_OID.getId()) == null) {
                Log.w(TAG, "Key attestation OID not found, skipping modification");
                return response;
            }
            ...
            blockKeyAttestationIfSpoofingGMS(3);
            return response;
        }
        return response;
    }
